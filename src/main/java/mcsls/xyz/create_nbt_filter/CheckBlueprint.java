...
public class CheckBlueprint {
    ...
    public void OnBlueprintUpload(BluePrintUploadEvent event)//蓝图上传成功的事件
    {
        ...
        String blueprintId = event.BlueprintId;//获取蓝图的 Id
        ...
        if (filter.VerifyBlueprint(event.BlueprintId)) {//校验蓝图
            table.inventory.setStackInSlot(1, SchematicItem.create(world.holderLookup(Registries.BLOCK), blueprintId, player.getGameProfile()
                    .getName()));//生成对应的蓝图
